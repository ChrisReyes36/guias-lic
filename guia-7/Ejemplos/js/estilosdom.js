...
function deleteRule() {
    ...
    if (styleSheet.cssRules.length > 0) {
        if (styleSheet.removeRule) styleSheet.removeRule(0);
        else if (styleSheet.deleteRule) styleSheet.deletRule(0);
    }
}
function addRule() {
    var styleSheet = document.styleSheets[0];
    ...
    if ("insertRule" in styleSheet) {
        styleSheet.insertRule(
            'h3 {text-align:center; font-family:"Century Gothic"; fontsize:18pt; color:Brown;}',
            4
        );
    } else if ("addRule" in styleSheet) {
        styleSheet.addRule(
            "h3",
            'text-align:center; font-family:"Century Gothic"; fontsize:18pt; color:Brown;',
            4
        );
    }
}
